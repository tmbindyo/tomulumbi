...
@section('title','Order '. $order->order_number.' Edit')
                ...
                <div class="main-card mb-3 card">
                    ...
                    <div class="card-header">
                        ...
                        <div class="card-body"><h5 class="card-title">Orders</h5>
                            <form method="post" action="{{ route('admin.order.update',$order->id) }}" autocomplete="off" class="form-horizontal form-label-left">
                                @csrf
                                ...
                                <div class="position-relative form-group">
                                    ...
                                    <select required="required" style="width: 100%" {{ $errors->has('contact') ? ' is-invalid' : '' }} name="contact" id="contact" class="contact-select form-control input-lg">
                                        ...
                                        @foreach($contacts as $contact)
                                            <option @if($contact->id == $order->contact_id) selected @endif value="{{$contact->id}}">{{$contact->first_name}} {{$contact->last_name}} @if($contact->organization) [{{$contact->organization->name}}] @endif</option>
                                        @endforeach
                                    </select>
                                    <i>expense account</i>
                                </div>
                                        ...
                                        <table class="table table-bordered" id = "expense_table">
                                            ...
                                            <tbody>
                                                ...
                                                @foreach($order->order_products as $product)
                                                    <tr>
                                                        <td>
                                                            <select onchange = "itemSelected(this)" data-placement="Select" name="item_details[{{$product_index}}][item]" required="required" style="width: 100%" {{ $errors->has('price_list') ? ' is-invalid' : '' }} id="price_list" class="price-list-select form-control input-lg">
                                                                <option>Select Price List</option>
                                                                @foreach($priceLists as $priceList)
                                                                    <option @if($product->price_list_id == $priceList->id) selected @endif value="{{$priceList->id}}" data-product-quantity = "-20" data-product-selling-price = "{{$priceList->price}}">{{$priceList->product->name}} [{{$priceList->size->size}} {{$priceList->sub_type->name}}] ({{$priceList->product->status->name}})</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td>
                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="position-relative form-group">
                                            ...
                                            <input name="subtotal" id="items-subtotal" readonly value="{{$order->subtotal}}" type="number" class="mb-2 form-control" {{ $errors->has('balance') ? ' is-invalid' : '' }} required="required">
                                            <i>subtotal</i>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="position-relative form-group">
                                            ...
                                            <input name="adjustment" id="adjustment-value" oninput="itemTotalChange()" type="number" class="mb-2 form-control" {{ $errors->has('balance') ? ' is-invalid' : '' }} required="required" value="{{$order->discount}}">
                                            <i>adjustment</i>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="position-relative form-group">
                                            ...
                                            <input name="grand_total" id="grand-total" type="number" class="mb-2 form-control" {{ $errors->has('balance') ? ' is-invalid' : '' }} required="required" value="{{$order->total}}" readonly>
                                            <i>total</i>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="has-warning">
                                    <textarea required name="customer_notes" placeholder="Notes" class="form-control" rows="7">{{$order->customer_notes}}</textarea>
                                </div>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            let due_date_set = {!! json_encode($order->expiry_date) !!};
            let due_date_substrings = due_date_set.split('-');
            var set_due_date = due_date_substrings[1] + '/' + due_date_substrings[2] + '/' + due_date_substrings[0];
            ...
            let expiry_date_set = {!! json_encode($order->expiry_date) !!};
    ...
    <script>
        ...
        var tableValueArrayIndex = {!! json_encode($product_index) !!};
