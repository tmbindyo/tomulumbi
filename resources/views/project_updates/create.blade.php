@extends('layouts.app', ['title' => __('Project update Management')])

@section('content')
    @include('users.partials.header', ['title' => __('Add Project update')])   
                ...
                <div class="card bg-secondary shadow">
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">{{ __('Project update Management') }}</h3>
                            </div>
                            <div class="col-4 text-right">
                                <a href="{{ route('project.project_update.index', $project->id) }}" class="btn btn-sm btn-primary">{{ __('Back to list') }}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('project.project_update.store', $project->id) }}" autocomplete="off" enctype="multipart/form-data">
                            @csrf
                            
                            <h6 class="heading-small text-muted mb-4">{{ __('Project update information') }}</h6>
                            <div class="pl-lg-4">
